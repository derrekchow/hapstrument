class StringMaterial {
  ...
  StringMaterial (boolean _isVertical, float _lowEnd, float _highEnd, float _location) {
  ...
  StringMaterial (boolean _isVertical, float _lowEnd, float _highEnd, float _location, float pixelsPerMeter, PVector deviceOrigin) {
    isVertical = _isVertical;
    lowEnd = _lowEnd;
    highEnd = _highEnd;
    location = _location;
...
static StringForce calculateForce(StringMaterial[] strings, PVector posEE) {
  ...
  return new StringForce(justCrossedString, (fWall.copy()).mult(-1));
}
